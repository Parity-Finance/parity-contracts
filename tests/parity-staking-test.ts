...
export async function runParityStakingTests(env: TestEnvironment) {
    let umi = env.umi;
    let poolManager = env.poolManager;
    let tokenManager = env.tokenManager;
    let keypair = env.keypair;
    let xMint = env.xMint;
    let xMintDecimals = env.xMintDecimals;
    let baseMint = env.baseMint;
    let baseMintDecimals = env.baseMintDecimals;
    let userX = env.userX;
    let userBase = env.userBase;
    let vaultStaking = env.vaultStaking;
    let testDepositCapAmount = env.testDepositCapAmount;
    let xMetadata = env.xMetadata;

    describe("Parity staking Tests", () => {
        it.only("Stake Pool is initialized!", async () => {
            ...
            assert.equal(
                ...
                BigInt(env.initialExchangeRateParityStaking)
            );
